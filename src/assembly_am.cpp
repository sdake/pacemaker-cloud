...
{
	AssemblyAm *a = (AssemblyAm*)user_data;
	a->check_state();
}
...
void
AssemblyAm::service_execute(struct pe_operation *op, std::string method,
...
void
AssemblyAm::resource_execute(struct pe_operation *op, std::string method,
			   qpid::types::Variant::Map in_args)
{
 ...
 */
void
AssemblyAm::op_history_insert(xmlNode *rsc, struct operation_history *oh)
{
	xmlNode *op;
	char key[255];
	char magic[255];
...
void
AssemblyAm::op_history_save(Resource* r, struct pe_operation *op,
...
void
AssemblyAm::op_history_del_by_resource(Resource* r)
{
...
void
AssemblyAm::insert_status(xmlNode *status)
{
...
uint32_t
AssemblyAm::check_state_online(void)
{
...
uint32_t
AssemblyAm::check_state_offline(void)
{
	uint32_t new_state = _state;
	if (_hb_state == HEARTBEAT_OK &&
...
void
AssemblyAm::state_offline_to_online(void)
{
	qb_loop_timer_handle th;

	qb_log(LOG_NOTICE, "AssemblyAm (%s) STATE_ONLINE.",
	       _name.c_str());
...
void
AssemblyAm::state_online_to_offline(void)
{
	_mh_rsc.disconnect();
	_mh_serv.disconnect();
	_mh_host.disconnect();
	...
	qb_log(LOG_NOTICE, "AssemblyAm (%s) STATE_OFFLINE.",
	       _name.c_str());
	_dep->assembly_state_changed(this, "failed", "Not reachable");
}

void
AssemblyAm::heartbeat_recv(uint32_t timestamp, uint32_t sequence)
{
	gdouble elapsed = 0;

	if (_hb_state != AssemblyAm::HEARTBEAT_OK) {
		_last_sequence = sequence;
		_hb_state = AssemblyAm::HEARTBEAT_OK;
	...
	if (sequence > (_last_sequence + 1)) {
		_hb_state = AssemblyAm::HEARTBEAT_SEQ_BAD;
...
AssemblyAm::AssemblyAm() :
	_hb_state(HEARTBEAT_INIT), Assembly()
{
...
AssemblyAm::~AssemblyAm()
{
	qb_log(LOG_DEBUG, "~AssemblyAm(%s)", _name.c_str());
}
